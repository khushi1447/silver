...
async function validateSeedData() {
  ...
  try {
    ...
    // Check if all products have categories (categoryId is required in schema, so this should be 0)
    ...
    const productsWithoutCategory = productCount - productsWithCategories;
    console.log(`   Products without category: ${productsWithoutCategory} ${productsWithoutCategory === 0 ? '✅' : '❌'}`);
    ...
    // Check for orphaned reviews (this should always be 0 due to foreign key constraints)
    const totalReviewsCheck = await prisma.review.count();
    const validReviews = await prisma.review.count({
      where: {
        AND: [
    ...
    const invalidReviews = totalReviewsCheck - validReviews;
    console.log(`   Invalid reviews: ${invalidReviews} ${invalidReviews === 0 ? '✅' : '❌'}`);
